...
spark=SparkSession.builder.appName("EmojiAggergator").config("spark.jars.packages", "org.apache.spark:spark-sql-kafka-0-10_2.12:3.2.0").getOrCreate()
...
agg_count_emoji=agg_emoji_rdd.agg(F.count("*").alias("count")).withColumn("agg_count",F.ceil(col("count")/1000))

#def print_batch(batch_df, batch_id):
#    print(f"Batch ID: {batch_id}")
#    batch_df.show(truncate=False)

query=agg_count_emoji.selectExpr("to_json(struct(*)) AS value").writeStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("subscribe","processed-emoji-data").output("update").start()
